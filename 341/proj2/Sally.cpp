   ...
   istrm(input_stream)  // use member initializer to bind reference
{
   ...
   symtab["=="] = SymTabEntry(KEYWORD,0,&doEQUALTO) ;
   symtab["!="] = SymTabEntry(KEYWORD,0,&doNOTEQUALTO) ;
...
void Sally::doSET(Sally *Sptr){
  ...
  //find if in symtab or not
  it = Sptr->symtab.find(p.m_text);
  if(it == Sptr->symtab.end()){
    Sptr->symtab[p.m_text] = SymTabEntry(VARIABLE, a.m_value);
  }
  else{
    cout << "That variable already exist" << endl;
  }
}

void Sally::doFETCH(Sally *Sptr){  Token p;
  map<string,SymTabEntry>::iterator it;

  if(Sptr->params.size() < 1){
    throw out_of_range("Need parameter for FETCH");
  }
  ...
  it = Sptr->symtab.find(p.m_text);
  if(it == Sptr->symtab.end()){
    cout << "Error, no such variable exist" << endl;
  }
  else{
    //make token 
    Sptr->params.push(Token(INTEGER, it->second.m_value, p.m_text)); 
  }
}
...
void Sally::doEXCLAMATION(Sally *Sptr){
  ...
  if(Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for EXCLAMATION");
  }
...
void Sally::doEQUALTO(Sally *Sptr){
  Token p, a;
...
void Sally::doNOTEQUALTO(Sally *Sptr){
  Token p, a;
...
void Sally::doIFTHEN(Sally *Sptr){
  ...
  if(p.m_value == 1){
    ...
    while(Sptr->ifThenCounter > 0){
      p = Sptr->nextToken();

      if(p.m_text == "ELSE"){
	Sptr->ifThenCounter--;
      }
      else if(p.m_text == "IFTHEN"){
	Sptr->ifThenCounter++;
      }
    }
  }
...
void Sally::doELSE(Sally *Sptr){
  ...
  if(Sptr->ranIfThen == false){
    return;
  }
  else{
    while(Sptr->elseCounter > 0){
      p = Sptr->nextToken();
      
      if(p.m_text == "IFTHEN"){
	Sptr->ifThenCounter++;
      }
      else if(p.m_text == "ENDIF"){
	Sptr->elseCounter--;
      }
    }
  }
